...
export default async function handler(req, res) {
  try {
    ...
    const interiorsProjectName = req.body.interiorsProjectName;

    // Check if project name is missing
    if (!interiorsProjectName) {
      console.log('Validation failed: Missing interiors project name');
      return res.status(400).json({ error: 'Project name is required' });
    }

    // Specify the Google Cloud Storage bucket and image filename
    const bucketName = 'jj-webapp1';
    const imageFileName = `${interiorsProjectName}.jpg`; 
