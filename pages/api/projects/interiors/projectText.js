...
export default async function handler(req, res) {
  if (req.method === "POST") {
    try {
      ...
      const interiorsProjectTextData = {
        name,
        description,
        timestamp,
      };
      ...
      await kv.set(`interiorsproject:${name}`, JSON.stringify(interiorsProjectTextData));

      res.status(200).json({ message: "Interiors project text data uploaded successfully" });
    } catch (error) {
      console.error(error);
