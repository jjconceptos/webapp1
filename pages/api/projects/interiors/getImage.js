...
export default async function handler(req, res) {
  try {
    ...
    console.log('Received request for project names list (getImage.js):', interiorsProjectNames);

    // Split the comma-separated project names into an array
    const interiorsProjectNamesArray = Array.isArray(interiorsProjectNames) ? interiorsProjectNames : interiorsProjectNames.split(',');

    // Specify the Google Cloud Storage bucket and image filename for each project
    const bucketName = 'jj-webapp1';
    const imageFileNames = interiorsProjectNamesArray.map((interiorsProjectName) => `${interiorsProjectName}.jpg`);
