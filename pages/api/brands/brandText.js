...
export default async function handler(req, res) {
  if (req.method === "POST") {
    try {
      ...
      // Assuming you use a unique key for each brand
      await kv.set(`brand:${name}`, JSON.stringify(brandTextData));

      res.status(200).json({ message: "Brand text data uploaded successfully" });
    } catch (error) {
      console.error(error);
