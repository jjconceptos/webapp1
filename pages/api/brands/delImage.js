...
export default async function handler(req, res) {
  try {
    ...
    console.log('Received a POST request to /api/brands/delImage');
    console.log('Request body:', req.body);

    // Extract brand name from the request
    const brandName = req.body.brandName;

    // Check if brand name is missing
    if (!brandName) {
      console.log('Validation failed: Missing furniture brand name');
      return res.status(400).json({ error: 'Brand name is required' });
    }

    // Specify the Google Cloud Storage bucket and image filename
    const bucketName = 'jj-webapp1';
    const imageFileName = `${brandName}.jpg`; 
