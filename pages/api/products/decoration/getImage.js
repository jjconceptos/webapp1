...
export default async function handler(req, res) {
  try {
    ...
    console.log('Received request for product names list (getImage.js):', decorationProductNames);

    // Split the comma-separated project names into an array
    const decorationProductNamesArray = Array.isArray(decorationProductNames) ? decorationProductNames : decorationProductNames.split(',');

    // Specify the Google Cloud Storage bucket and image filename for each project
    const bucketName = 'jj-webapp1';
    const imageFileNames = decorationProductNamesArray.map((decorationProductName) => `${decorationProductName}.jpg`);
