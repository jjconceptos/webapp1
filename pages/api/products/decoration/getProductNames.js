...
export default async function handler(req, res) {
  ...
  if (requestType === "GET") {
    try {
      ...
      const decorationProductNames = await kv.get('decorationproductnames:');

console.log("Received GET Request for decoration product names");

if (decorationProductNames) {
  console.log("Fetched Decoration Project Names List:", decorationProductNames);
  res.status(200).json(decorationProductNames);
} else {
