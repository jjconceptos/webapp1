...
export default async function handler(req, res) {
  try {
    ...
    console.log('Received request for product names list (getImage.js):', productNames);

    // Split the comma-separated project names into an array
    const productNamesArray = Array.isArray(productNames) ? productNames : productNames.split(',');

    // Specify the Google Cloud Storage bucket and image filename for each project
    const bucketName = 'jj-webapp1';
    const imageFileNames = productNamesArray.map((productName) => `${productName}.jpg`);
