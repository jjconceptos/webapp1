...
const interiorsProjectForm = ({ onSubmit, onInteriorsProjectAdded, interiorsProjects }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const imageResponse = await fetch('/api/projects/interiors/projectImage', {
        method: 'POST',
        body: formData,
        headers: {
      ...
      if (imageResponse.ok) {
        ...
        const textResponse = await fetch('/api/projects/interiors/projectText', {
          method: 'POST',
        ...
        if (textResponse.ok) {
          ...
          const interiorsProjectNamesResponse = await fetch('/api/projects/interiors/projectNames', {
            method: 'POST',
            body: JSON.stringify({ interiorsProjectNames: [name] }), // Send just the project name as an array
            headers: {
              'Content-Type': 'application/json',
            },
          });
...
export default interiorsProjectForm;
